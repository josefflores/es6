...
describe('MicRegister', function () {
    ...
    describe('Object is created', function () {
        ...
        it('Busses have been initialized to 0', function () {
            expect(mr.bus.get('a', '1')).to.equal(mu.const.get('ZERO'));

            expect(mr.bus.get('a', '0')).to.equal(false);

            expect(mr.bus.get('b', '1')).to.equal(mu.const.get('ZERO'));
    ...
    describe('Registers are readable', function () {
        ...
        it('Registers can be read into the a bus', function () {
            ...
            expect(mr.bus.get('a', '1')).to.equal(mu.const.get('POS_ONE'));

            expect(mr.register.read('negOne', 'a')).to.equal(true);
            expect(mr.bus.get('a', '1')).to.equal(mu.const.get('NEG_ONE'));
        ...
        it('Registers can be read into the b bus', function () {
            ...
            expect(mr.bus.get('b', '1')).to.equal(mu.const.get('POS_ONE'));

            expect(mr.register.read('negOne', 'b')).to.equal(true);
            expect(mr.bus.get('b', '1')).to.equal(mu.const.get('NEG_ONE'));
    ...
    describe('Registers are writable', function () {
        ...
        it('Registers can be loaded from the c bus', function () {
            ...
            expect(mr.bus.get('a', '1')).to.equal(mu.const.get('ZERO'));
            expect(mr.bus.set('c', mu.const.get('POS_ONE'))).to.equal(true);
