...
describe('MicClock', function () {
    ...
    describe('#getTime()', function () {
        ...
        it('A new MicClock should initialize to { cycle:0, subCycle:0}', function () {
            expect(micClock.getTime().cycle).to.equal(0);
            expect(micClock.getTime().subCycle).to.equal(0);
        });
    });
    ...
    describe('#tick()', function () {
        ...
        it('Transitions through subcycles into a cycle, 5 ticks cycle to { cycle:2, subCycle:1}', function () {
            ...
            var tickTest = function tickTest(cycle, subCycle) {
                micClock.tick();
                expect(micClock.getTime().cycle).to.equal(cycle);
                expect(micClock.getTime().subCycle).to.equal(subCycle);
            };
