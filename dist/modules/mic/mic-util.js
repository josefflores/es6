...
function MicUtil(opt) {
    ...
    function andBitString(leftOp, rightOp) {
        return leftOp.split('').map(function (character, index) {
    ...
    function padLeft(input) {
        ...
        if (input >= 0) {
            ...
            //	Create padding
            pad = Array(size).join(input >= 0 ? '0' : '1');
    ...
    function shift(direction, input) {
        ...
        if (direction === 'L') {
            return (input + right).slice(1, input.length + 1);
        } else if (direction === 'R') {
            return (left + input).slice(0, -1);
        }
