...
exports.default = gulpTaskWrapper;
...
class GulpMan {
    ...
    task(name, func) {
        ...
        return func ? _priv.task(name, func) : _priv.task(name);
    }
    ...
    menu(done) {
        ...
        function printSection(label, prop) {
            ...
            let t = _priv.t;
            let title = _chalk2.default[_priv.styleTitle](`${ label.toUpperCase() }`);

            //  Generate section string
            if (prop == 'title' && item[prop]) {
                _priv.msg.push(`\n${ t }${ item[prop].toUpperCase() }(${ _package2.default.version })${ t.repeat(2) }${ label }\n`);
            } else if (item[prop]) {
        ...
        function printTasks(obj) {
            ...
            if (obj) {
                ...
                obj.sort((a, b) => {
                    ...
                    if (b.name == 'default' || a.name > b.name) return 1;
                    if (a.name < b.name) return -1;
                    return 0;
                })
                //  Add sorted tasks
                .forEach(o => {
                    let title = _chalk2.default[_priv.StyleTask](`${ o.name }`);
                    _priv.msg.push(`\n${ t.repeat(2) }${ title }\n${ t.repeat(3) }${ o.desc }\n`);
                });
            }
        };
        ...
        _priv.menuSettings = global.app.setOpts({
            name: 'gulpfile',
            usage: `gulp ${ _chalk2.default['bold']('[OPTIONS]') }`,
            desc: 'A Gulp task runner.',
            bugs: _package2.default.bugs,
            also: _package2.default.homepage,
            title: _package2.default.name,
            copy: _package2.default.license,
            auth: _package2.default.author,
            debug: false
        }, _priv.info);
...
function gulpTaskWrapper(gulp) {
    ...
    return option => {
        ...
        if (option.desc) {
            ...
            if (option.man === true || option.name === 'default') {
                gulpMan.add(option);
            }
        }
